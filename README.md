...
| Snapshot recovery | Saves the host world, automatically reconnects Steam and direct-IP guests, verifies snapshot bytes and reloads everyone before resuming. | Turns recoverable desyncs into a pause and shared reload; includes a manual **Resync from host** button. |
...
| Reconnect grace | Unexpected Steam or direct-IP drops trigger shared snapshot recovery with a 30-second reconnect window and continue/cancel controls. | Reduces manual recovery steps and lets the host continue with available players. |
...
Earlier stability previews were playtested by the fork owner. Preview 15 validates the corrected localization assets and includes the Steam transport changes; live two-account Steam playtesting remains pending. It does not eliminate every possible desync or guarantee compatibility with all other mods. Performance improvements reduce specific mod overhead; they are not a measured promise of higher game FPS.

See the [reconnect guide](RECONNECT-GRACE.md) and [status panel controls](CONNECTION-STATUS.md) for the latest features.
...
Keep other gameplay mods and their settings consistent between players. The checks cannot prove that every matching third-party mod is deterministic. Back up your save before changing your mod setup.
...
**Automatic host snapshot recovery** is enabled by default and controlled by the host. Steam, direct-IP and mixed sessions are supported. Everyone reloads the same host snapshot, and the host waits for all guests before restoring the previous speed. A full reload can take longer for large settlements. Use **Resync from host** in the options menu to test the flow. See [recovery behavior, safeguards and test steps](SNAPSHOT-RECOVERY.md).

## Validation and reporting problems

See [Preview 15 validation](STEAM-INVITES.md) for the current test results. The Release Steam build and automated suites cover networking, replay, activity, mod compatibility, rolling diagnostics, recovery, simulation timing and input cleanup, including real loopback TCP tests. Live two-account Steam/Unity validation of Preview 15 remains to be completed.
