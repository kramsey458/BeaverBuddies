...
SteamListener Host(){var l=new SteamListener(123);l.Start();return l;}
...
Test("Recovery retains lobby members and does not create another lobby",()=>{
 var l=Host();int creates=SteamMatchmaking.Creates,leaves=SteamMatchmaking.Leaves;ulong id=l.DetachLobby();l.Stop();Check(SteamMatchmaking.Leaves==leaves);
 var fresh=new SteamListener(id);fresh.Start();Check(SteamMatchmaking.Creates==creates&&fresh.LobbyID.m_SteamID==123);fresh.Stop();Check(SteamMatchmaking.Leaves==leaves+1);
});
...
Test("Large compatibility profiles are fragmented to the relay stream limit",()=>{
 SteamNetworkingSockets.AutoPair=true;var host=Host();var guest=new SteamRelaySocket(new(1));try{
 guest.ConnectAsync().GetAwaiter().GetResult();var accepted=host.AcceptClient();
 string identity=string.Concat(Enumerable.Range(0,2200).Select(_=>Guid.NewGuid().ToString("N")));
 var a=Task.Run(()=>CompatibilityHandshake.Run(accepted,identity,true));var b=Task.Run(()=>CompatibilityHandshake.Run(guest,identity,false));
 Check(Task.WaitAll(new[]{a,b},5000));
 }finally{guest.Close();host.Stop();Cleanup();}
});
Test("Real TimberNet sends a multi-megabyte save over relay without Unity receive pumping",()=>{
 ...
 var server=new TimberServer(listener,()=>Task.FromResult(bytes),null){CompatibilityIdentity="same",ReconnectTickets=new()};
 var client=new TimberClient(new SteamRelaySocket(new(1))){CompatibilityIdentity="same",UseReconnectHandshake=true};
 byte[] loaded=null;string error=null;client.OnMapReceived+=b=>loaded=b;client.OnError+=e=>error=e;
 try{server.Start();client.Start();Wait(()=>{server.Update();client.Update();return loaded!=null||error!=null;},8000);Check(error==null,error);Check(loaded.SequenceEqual(bytes));Check(client.ReconnectToken?.Length==64);}
...
Test("Recovery accepts an original Steam identity even after Steam drops lobby membership",()=>{
 var l=new SteamListener(123,new ulong[]{2});l.Start();SteamMatchmaking.Members.Remove(new(2));
 try{int before=SteamNetworkingSockets.Accepts;var h=SteamNetworkingSockets.New(new(2),SteamNetworkingSockets.Listener);
 SteamNetworkingSockets.Signal(h,k_ESteamNetworkingConnectionState_Connecting);Check(SteamNetworkingSockets.Accepts==before+1);}
 finally{SteamMatchmaking.Members.Add(new(2));l.Stop();}
});
Test("Recovery refuses newly invited identities that were not in the old session",()=>{
 var l=new SteamListener(123,new ulong[]{3});l.Start();
 try{int before=SteamNetworkingSockets.Accepts;var h=SteamNetworkingSockets.New(new(2),SteamNetworkingSockets.Listener);
 SteamNetworkingSockets.Signal(h,k_ESteamNetworkingConnectionState_Connecting);Check(SteamNetworkingSockets.Accepts==before);}
 finally{l.Stop();}
});
...
Test("Mixed Steam and real TCP guests share ordered events and reconnect tickets",()=>{
 SteamNetworkingSockets.AutoPair=true;var tcp=new LocalListener();var steam=new SteamListener(123);
 var server=new TimberServer(new MultiSocketListener(tcp,steam),()=>Task.FromResult(new byte[]{1,2,3}),null){CompatibilityIdentity="mixed",ReconnectTickets=new()};
 TimberClient a=null,b=null;try{
 server.Start();a=new(new SteamRelaySocket(new(1))){CompatibilityIdentity="mixed",UseReconnectHandshake=true};
 b=new(new TCPClientWrapper("127.0.0.1",tcp.Port)){CompatibilityIdentity="mixed",UseReconnectHandshake=true};
 ...
 Check(a.ReconnectToken.Length==64&&b.ReconnectToken.Length==64&&a.ReconnectToken!=b.ReconnectToken);
 }finally{a?.Close();b?.Close();server.Close();Cleanup();}
});

Test("Relay guest reloads a fresh host snapshot using the same lobby and recovery ticket",()=>{
 SteamNetworkingSockets.AutoPair=true;var listener=new SteamListener(123);
 var server=new TimberServer(listener,()=>Task.FromResult(new byte[]{1,2,3}),null){CompatibilityIdentity="recovery",ReconnectTickets=new()};
 TimberClient client=null;
 try{
  server.Start();client=new(new SteamRelaySocket(new(1))){CompatibilityIdentity="recovery",UseReconnectHandshake=true};
  byte[] loaded=null;client.OnMapReceived+=b=>loaded=b;client.Start();
  Wait(()=>{server.Update();client.Update();return loaded!=null&&server.ReconnectTickets.Export().Length==1;},5000);
  string ticket=client.ReconnectToken;var roster=server.ReconnectTickets.Export();var ids=listener.ExportPeers();ulong lobby=listener.DetachLobby();
  client.Close();server.Close();loaded=null;
  listener=new SteamListener(lobby,ids);
  server=new(listener,()=>Task.FromResult(new byte[]{9,8,7,6}),null){CompatibilityIdentity="recovery",ReconnectTickets=new(roster,"new-world","digest")};
  server.Start();client=new(new SteamRelaySocket(new(1))){CompatibilityIdentity="recovery",UseReconnectHandshake=true,ReconnectToken=ticket};
  client.OnMapReceived+=b=>loaded=b;client.Start();
  Wait(()=>{server.Update();client.Update();return loaded!=null;},5000);
  Check(loaded.SequenceEqual(new byte[]{9,8,7,6})&&client.ReconnectToken==ticket&&client.RecoveryId=="new-world"&&client.RecoveryDigest=="digest");
 }finally{client?.Close();server.Close();Cleanup();}
});
...
sealed class InviteFixture {
 ...
 public InviteFixture(){
  BeaverBuddies.IO.EventIO.IsNull=true;BeaverBuddies.Connect.SnapshotResyncService.Active=false;SteamGuestLobby.Leave();
