# Stability fork: cumulative changelog

Newest first. Every entry is cumulative: a build includes everything in the entries below it.
For a plain-language summary of what changed and how well each fix is confirmed, see the
[README](README.md).

This fork branched from upstream BeaverBuddies `v1.1` at commit
`a13b1f20dacb6e30efa967cc8ac83e73779c0755` (24 August 2026) and is built against
Timberborn 1.1.2.4.

Version numbers: the `1.1.0-stability.N` builds were the numbered "Preview N" builds.
**1.1.1** is the same code as Preview 17 with a release version number, and **1.1.3** is the
current release. There was no 1.1.2 release (see the `1.1.2-steam.1` entry), and Previews
9-16 are not part of this line.

## 1.1.3 — Steam invites and connection panel

The current release. Built on 1.1.1. It includes the Steam relay invites first tested as
`1.1.2-steam.1` (see the entry below), so one build has both. Merged into `main` in
[PR #2](https://github.com/kramsey458/BeaverBuddies-Multiplayer-Stability-Fork/pull/2). See
`STEAM-INVITES.md` and `CONNECTION-PANEL.md`.
...
- 87 StabilityTests, 55 RuntimeChecks and 2 Python checks pass; Release Steam and non-Steam
  builds succeed with no warnings.
- The Steam transport is tested against a fake Steam network that fails any Steam call made
  ...
  corrupting one byte in the Steam path makes them fail. The panel's ping and roster feed is
  tested over the Steam transport as well as direct connections.
- The layer that calls the real Steam client is not covered by the automated checks. It has
  been confirmed in real two-player playtests over Steam, and the fork owner reported that
  1.1.3 works flawlessly, including the connection panel.

**Known limits**

- Everyone in a session must run the identical build. Other mods and settings are not
  compared.
- Text added in this release is English only; other languages fall back to English.
- One tester reported the game window losing focus when loading a save on the
  `1.1.2-steam.1` test build. It did not reproduce and was never explained.

## 1.1.2-steam.1 — Steam relay invites (test build)

A test build on 1.1.1, never released as an official version: there was no 1.1.2. Its
changes are part of 1.1.3 and are described there. It was confirmed working between two
real Steam accounts, and is marked superseded by 1.1.3 on its release page.
...
The fork owner's two-player playtests confirmed: the animation crash fix (Preview 1), the
badtide desync fix (Preview 4), that Previews 5 and 6 play well, the player activity
indicators (1.1.1) and Steam invites with the connection panel (1.1.3). They confirm the
reported issues and that those builds play well, not universal determinism.

Not confirmed in a live session: the demolition-selection crash fix (1.1.1), equal-distance
demolition tie-breaking (Preview 7) and the stuck-controls recovery (Preview 8). Only two
players have been tested. Other game versions and combinations of mods may still have
unrelated problems. No Housing Optimize changes are included. Upstream authorship and GPL
licensing are preserved in `License.txt` and the repository history.
